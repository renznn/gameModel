...
export default class ListView extends cc.Component {
  ...
  public onLoad() {
    if (this.scrollView) {
      ...
      if (this.horizontal) {
        ...
        this.content.y =
          this.content.getParent().height * this.content.getParent().anchorY;
      }
    } else {
    ...
    if (this.horizontal) {
      this._itemsVisible = Math.ceil(
        this.content.getParent().width / this._itemWidth
      );
    } else {
      this._itemsVisible = Math.ceil(
        this.content.getParent().height / this._itemHeight
      );
    }
  ...
  public notifyUpdate(updateIndex?: number[]) {
    ...
    if (this.horizontal) {
      this.content.width =
        this.adapter.getCount() * (this._itemWidth + this.spacing) +
        this.spacing;
    } else {
      this.content.height =
        this.adapter.getCount() * (this._itemHeight + this.spacing) +
  ...
  private _layoutVertical(child: cc.Node, posIndex: number) {
    ...
    child.setPosition(
      0,
      -child.height * (0.5 + posIndex) - this.spacing * (posIndex + 1)
    );
  }
  ...
  private _layoutHorizontal(child: cc.Node, posIndex: number) {
    ...
    this._filledIds[posIndex] = posIndex;
    child.setPosition(
      child.width * (child.anchorX + posIndex) + this.spacing * posIndex,
      0
    );
  }
  ...
  private updateView(startIndex) {
    ...
    // 比实际元素多3个.
    let itemEndIndex =
      itemStartIndex + this._itemsVisible + (this.spawnCount || 2);
    ...
    const recyles = this.getRecycleItems(
      itemStartIndex - (this.spawnCount || 2),
      itemEndIndex
    );
  ...
  private checkNeedUpdate(): number {
    ...
    let scroll = this.horizontal
      ? -this.content.x -
        this.content.getParent().width * this.content.getParent().anchorX
      : this.content.y -
        this.content.getParent().height * this.content.getParent().anchorY;
    let itemStartIndex = Math.floor(
      scroll /
        ((this.horizontal ? this._itemWidth : this._itemHeight) + this.spacing)
    );
